...
int main(int argc, char *argv[]) {
    ...
    struct addrinfo addrCriteris;
    memset(&addrCriteris, 0, sizeof(addrCriteris));
    addrCriteris.ai_family = AF_UNSPEC;
    addrCriteris.ai_socktype = SOCK_DGRAM;
    addrCriteris.ai_protocol = IPPROTO_UDP;

    struct addrinfo *servAddr;
    int rtnVal = getaddrinfo(server, service, &addrCriteris, &servAddr);
    if (rtnVal != 0)
        ...
        if (errno == EINTR) {
            if (tries < MAXTRIES) {
                ...
                if (numBytes < 0)
                    ...
                    DieWithUserMessage("sendto() error", "sent expected number of bytes");
